...
import { convertToBuffer, isEmptyData } from './helpers';
...
const MAX_HASHABLE_LENGTH = Math.pow(2, 53) - 1;

export class Sha256 implements IHashFunction {
  public static readonly BLOCK_SIZE = BLOCK_SIZE;
  public state: any;
  public temp: any;
  public buffer: any;
  public bufferLength: any;
  public bytesHashed: any;
  ...
  public update(val: Uint8Array) {
    ...
    while (byteLength > 0) {
      this.buffer[this.bufferLength++] = val[position++];
      byteLength--;
  ...
  private hashBuffer() {
    ...
    for (var i = 0; i < BLOCK_SIZE; i++) {
      ...
      var t1 =
        ((((((state4 >>> 6) | (state4 << 26)) ^ ((state4 >>> 11) | (state4 << 21)) ^ ((state4 >>> 25) | (state4 << 7))) +
          ((state4 & state5) ^ (~state4 & state6))) |
          0) +
          ((state7 + ((KEY[i] + this.temp[i]) | 0)) | 0)) |
        0;
      var t2 =
        ((((state0 >>> 2) | (state0 << 30)) ^ ((state0 >>> 13) | (state0 << 19)) ^ ((state0 >>> 22) | (state0 << 10))) +
